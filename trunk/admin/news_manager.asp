...
<%
	...
	if ufomail_request("Form","action")<>"" then
		...
		do while not rs.eof 
			rs("site_news")=ufomail_request("Form","site_news_" & rs("id"))
			rs.update
			rs.movenext
		loop
		rs.close
...
<!--功能标题-->
<div class="page-title">首页公告栏管理</div>
<%if return_info<>"" then%>
<div class="return-info"><%=return_info%></div>
<%end if%>
<div class="help-info">说明： <br>
  1、在此版录入的内容将会出现在首页的公告栏内.并以滚动的型式展示!<br>
</div>
...
<table width="95%" border="1" cellpadding="6" cellspacing="0" bordercolor="#CCCCCC" class="table-cell">
		...
		%>
		<tr> 
        <td width="100" align="right" bgcolor="#f2f8ff"><%=rs("language")%>公告：</td>
          <td><%
				
				oFCKeditor.Value = rs("site_news")
				oFCKeditor.Create "site_news_" & rs("id")
				%></td>
        </tr>
		<%
			end if
