	...
	<xsl:template match="/">
		<html>
			...
			<head>
				...
				<link href="template/site/css/index17.css" rel="stylesheet" type="text/css" media="screen"/>
				<!-- 打印时候使用的样式表 -->
		...
		<xsl:if test="count(page/product_list/product)=1">
			<xsl:for-each select="page/product_list/product">
				...
				<div class="product-pic">
					<xsl:if test="pic!=''"><a href="{picture}" target="_blank"><img src="{pic}" alt="{title}" width="200px"/></a></xsl:if>	
		...
		<xsl:if test="count(page/product_list/product)>1">
			...
			<xsl:for-each select="page/product_list/product">
				<div class="product_list">
					...
					<xsl:if test="pic!=''">
						<a href="index.asp?sitepage={//cur_page}&amp;contentid={content_id}" style="border:3px solid #ccc;background:#3E3646;" target="_blank">
						<!--根据系统设置，设置图片的大小-->
