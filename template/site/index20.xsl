...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="/">
		<html>
			...
			<head>
				...
				<link href="template/site/css/index20.css" rel="stylesheet" type="text/css" media="screen"/>
				<!-- 打印时候使用的样式表 -->
				<link href="template/site/css/print.css" rel="stylesheet" type="text/css" media="print"/>
				<SCRIPT LANGUAGE="JScript" SRC="/template/site/images/16/menus.js"></SCRIPT>
			</head>
			<!--head完成-->		
			<body>
			<div class="content">
					...
					<div class="page-menu" ID="divMenuBar">
						...
						<xsl:for-each select="page/sitemap/pagename">
							<xsl:if test="page_id=/page/parent_page">
								<li><a href="index.asp?sitepage={page_id}" id="active-link" class="clsMenuBarItem">
								<xsl:attribute name="title">tdMenuBarItem<xsl:value-of select="page_id" /></xsl:attribute>
								<span><xsl:value-of select="title"/></span></a></li>
							</xsl:if>
							<xsl:if test="page_id!=/page/parent_page">
								<li><a href="index.asp?sitepage={page_id}" class="clsMenuBarItem">
				...
				<div class="page-body">
					<div class="body-left">
						...
						<!--搜索框架-->
						<div class="page-nav">
							<div class="body-left-title"><xsl:value-of select="/page/language_pack/text_product_search"/></div>
							<div class="page-search">
							<form action="index.asp" name="search_form" method="get">
								<input name="keyword" id="keyword" type="text"/><input type="image" src="/template/site/images/1/search.jpg" name="action" value="search"/>
							</form>
							</div>
						</div>
						<!--二级导航栏-->
				...
				<!--创建子菜单-->
				<xsl:for-each select="page/sitemap/pagename[sitemap]">
					  <div class="hidden-menu">
					  <xsl:attribute name="ID">divMenu<xsl:value-of select="page_id" /></xsl:attribute>
				...
				<div class="page-foot"><span><xsl:value-of select="//foot_info" disable-output-escaping="yes"/></span></div>
				<!--页面底部广告代码-->
		...
		<xsl:if test="count(page/product_list/product)=1">
			<xsl:for-each select="page/product_list/product">
				...
				<div class="product-pic">
					<xsl:if test="pic!=''"><a href="{picture}" target="_blank"><img src="{pic}" alt="{title}"/></a></xsl:if>	
					<xsl:if test="pic=''"><img src="template/site/images/no_pic.gif" alt="{//site_name}"/></xsl:if>
				</div>
		...
		<xsl:if test="count(page/product_list/product)>1">
			...
			<xsl:for-each select="page/product_list/product">
				<div class="product_list">
					...
					<xsl:if test="pic!=''">
						<a href="index.asp?sitepage={//cur_page}&amp;contentid={content_id}" style="border:3px solid #ccc;background:#f2f3ff;" target="_blank">
						<!--根据系统设置，设置图片的大小-->
						<xsl:if test="//para_pic_width='' and //para_pic_height=''">
						<img src="{pic}" alt="{title}" width="155px"/>
						</xsl:if>
						<xsl:if test="//para_pic_width='' and //para_pic_height!=''">
					...
					<a href="index.asp?sitepage={//cur_page}&amp;contentid={content_id}" target="_blank" style="padding:4px;">
	...
	<xsl:template name="lyb">
		...
		<form action="lyb_save.asp" name="form1" method="post" id="lyb-form"><a name="ly"/>
			<table border="0" width="440px" cellpadding="4" cellspacing="0">
				<tr>
	...
	<xsl:template name="guestbook">
		...
		<form action="http://qiweb.cn/server/guestbook_save.asp" name="form1" method="post" id="lyb-form"><a name="ly"/>
			<table border="0" width="540px" cellpadding="4" cellspacing="0">
				<tr>
					<th><xsl:value-of select="/page/language_pack/text_add_lyb_name"/></th>
					<td><input type="text" name="user_id" style="width:100" class="inpt-text" value=""/>*</td>
				</tr>
	...
	<xsl:template name="album">
		...
		<xsl:for-each select="page/album_list/album">
			<div class="album_list">
				<a href="{pic}" rel="lightbox[qiweb]" title="{title}{content}" target="pic">
				<!--根据系统设置，设置图片的大小-->
				<xsl:if test="//para_pic_width='' and //para_pic_height=''">
				<img src="{pic}" alt="{title}" width="150px"/>
				</xsl:if>
				<xsl:if test="//para_pic_width='' and //para_pic_height!=''">
				<img src="{pic}" alt="{title}" height="{//para_pic_height}px"/>
				</xsl:if>
	...
	<xsl:template name="filelist">
		...
		<xsl:if test="count(page/software_list/software)=1">
			<xsl:for-each select="page/software_list/software">
				...
				<div class="product-pic">
					<xsl:if test="pic!=''">
						<a href="{pic}" target="_blank"><img src="{pic}" alt="{title}" width="200px"/></a>
					</xsl:if>
					<xsl:if test="pic=''"><img src="template/site/images/no_pic.gif" alt="{//site_name}"/></xsl:if>
				</div>
				<!--此处显示本地下载，其它下载按钮-->
