...
Dim key
'Partner 和 交易安全校验码
partner=db_getvalue("setup_name='alipay_userid'","sys_setup","setup_value")   'partner合作伙伴id
key = db_getvalue("setup_name='alipay_userkey'","sys_setup","setup_value")  'partner账户的支付宝安全校验码
'ATN 校验地址 
'*******************************************************************
...
sql="update po_basic set po_status=4 where PO_NO='"&trim(order_no)&"'"
conn.execute sql
